...
function vocalinfo_action(){
	...
	switch($_['action']){
		...
		case 'vocalinfo_wikipedia':
			 ...
			 $url = 'https://fr.wikipedia.org/w/api.php?action=query&format=json&titles='.$_['word'].'&prop=extracts&exintro=&explaintext=&redirects=1';
//'http://fr.wikipedia.org/w/api.php?action=query&format=json&titles='.$_['word'].'&prop=extracts&exintro=&explaintext=&redirects=1';
                                $ch = curl_init();
                                ...
                                $affirmation = $content; // strip_tags(str_replace('&#160;', ' ', $content)); // '&#160;' is a space, but is not recognized by yana trying to read "160"
